   ...
   begin
      case Comb is
         ...
         when K =>
            ...
            SK.Cells.Set_Cdr (Cells, Args (2), SK.Cells.Cdr (Cells, Args (1)));
            return SK.Cells.Cdr (Cells, Args (1));
         when S =>
            ...
            SK.Cells.Set_Car (Cells, Ts (1), SK.Cells.Cdr (Cells, Args (2)));
            SK.Cells.Set_Car (Cells, T, Ts (2));
            SK.Cells.Set_Cdr (Cells, Ts (2), X);
            SK.Cells.Set_Car (Cells, Ts (2), SK.Cells.Cdr (Cells, Args (1)));
         ...
         when Sd =>
          ...
          when Bd =>
            ...
            SK.Stack.Drop (Cells, 4);
            if Debug_Combinators then
               Ada.Text_IO.Put_Line ("B* --> " & SK.Images.Image (Cells, T));
            end if;
            return T;
         when Cd =>
            SK.Cells.Allocate (Cells, O_Application, Ts (1 .. 2));
            ...
            SK.Stack.Drop (Cells, 4);
            if Debug_Combinators then
               Ada.Text_IO.Put_Line ("C' --> " & SK.Images.Image (Cells, T));
            end if;
            return T;
      end case;
